...
const RegisterSchema = LoginSchema.extend({
	username: z
		.string("register.errors.invalidUsername")
	...
	password: z
		...
		.refine(
			...
			"register.errors.passwordLowercase",
		)
		.refine(
			(password) => /[A-Z]/.test(password),
			"register.errors.passwordUppercase",
		)
		.refine(
			(password) => /[1-9]/.test(password),
			"register.errors.passwordDigit",
		)
		.refine(
			(password) => /[$&+,:;=?@#|'<>.^*()%!-]/.test(password),
			"register.errors.passwordSpecialCharacter",
		),
...
const auth = new Elysia({ prefix: "/auth" })
	.post("/login", async ({ body, status, cookie }) => {
		...
		if (!validatedBody.success) {
			log(
				"Auth",
				`Authentication failed for user ${(body as any).email}: Validation failed. Stack trace below.`,
			);
		...
		if (
			...
			log(
				"Auth",
				`Failed authentication for email ${validatedBody.data.password}: Incorrect email or password.`,
			);
			return status(401, {
				code: "VALIDATION_FAILED",
				errors: [
	...
	.get("/session", async (ctx) => {
		const cookie =
			ctx.cookie[Bun.env.NODE_ENV === "production" ? "__Host-Token" : "token"];
		if (!cookie) return ctx.status("Unauthorized");
		const account = await db.query.accounts.findFirst({
			where: (accounts, { eq }) => eq(accounts.token, cookie.value as string),
			with: {
				user: true,
			},
		});
		...
		const settings = await db.query.accountSettings.findFirst({
			where: (accountSettings, { eq }) =>
				eq(accountSettings.accountId, account.id),
		});
		return {
			user: account.user,
			account: {
	...
	.delete("/clear-db", async (ctx) => {
		if (Bun.env.NODE_ENV === "production")
			return ctx.status(
				...
				"This endpoint can only be used from a development environment.",
			);
	...
	.post("/register", async ({ body, status, cookie, server }) => {
		...
		if (!validatedBody.success) {
			log(
				"Auth",
				`Failed registering user with email ${(body as any).email}. Validation failed. Stack trace below.`,
			);
			console.log(validatedBody.error.issues);
			return status(400, {
				code: "VALIDATION_FAILED",
		...
		if (Number(Bun.env.REGISTRATION_CLOSED)) {
			if (!validatedBody.data.inviteCode)
				return status(401, {
			...
			const code = await db.query.inviteCodes.findFirst({
				where: (codes, { eq }) =>
					eq(codes.code, validatedBody.data.inviteCode as string),
			});
			if (!code || code.used)
				return status(404, {
		...
		if (account) {
			log(
				"Auth",
				`Failed to register user with email ${validatedBody.data.email}: Email already exists.`,
			);
			return status(409, {
		...
		if (validatedBody.data.password !== validatedBody.data.confirmPassword) {
			log(
				"Auth",
				`Failed to register user with email ${validatedBody.data.email}: Password does not match confirm password.`,
			);
			return status(400, {
		...
		const user = await db.query.users.findFirst({
			where: (users, { eq, and }) =>
				and(
					eq(users.username, validatedBody.data.username),
					eq(users.tag, tag),
				),
		});
		if (user) {
			log(
				"Auth",
				`Failed to register user with email ${validatedBody.data.email}: User/Tag combination ${validatedBody.data.username}/${tag} already exists.`,
			);
			return status(409, {
		...
		try {
			...
			if (Number(Bun.env.REGISTRATION_CLOSED)) {
				inviteCode = await db
					...
					.where(eq(inviteCodes.code, validatedBody.data.inviteCode as string))
					.returning();
				server?.publish(
					"admins",
					JSON.stringify({
			...
			log(
				"Auth",
				`Registering user with email ${validatedBody.data.email} succeeded. Assigned ID: ${userId}`,
			);
			if (Bun.env.NODE_ENV !== "production") {
			...
			log(
				"Auth",
				`An error occurred while trying to register user with email ${validatedBody.data.email}. Stack trace:`,
			);
			console.error(error);
			return status(500, {
