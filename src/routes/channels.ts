...
import { and, eq, gt, type InferSelectModel, lt } from "drizzle-orm";
...
async function getMessages(
	...
	let q: any = eq(messages.channelId, channel);
...
export default new Elysia({ prefix: "/channels" }).use(ip()).guard(
	{
		beforeHandle(ctx) {
			if (
				!isAuthenticated(
		...
		_app.group("/:id", (app) =>
			app
				.resolve(async (ctx) => {
					...
					const guildMember = await db.query.guildMembers.findFirst({
						where: (guildMembers, { eq, and }) =>
							and(
								eq(guildMembers.userId, user?.id as string),
				...
				.post(
					"/messages",
					async (ctx) => {
						const data = messageCreateBody.safeParse(
							JSON.parse(ctx.body as any),
						);
						if (!data.success) {
					...
					},
					{
						beforeHandle: async (ctx) => {
							...
							const { limited, retryAfter } = await rateLimit(
								ctx.ip,
								(ctx.channel?.rateLimitPerUser as number) > 0 ? 1 : 50,
								(ctx.channel?.rateLimitPerUser as number) > 0
									? (ctx.channel?.rateLimitPerUser as number)
									: 1,
				...
				.patch(
					"/messages/:messageId",
					async (ctx) => {
						const data = messageUpdateBody.safeParse(
							JSON.parse(ctx.body as string),
						);
						if (!data.success) {
							console.log(data.error);
							return ctx.status("Bad Request", {
						...
						const message = await db.query.messages.findFirst({
							where: (messages, { eq }) =>
								eq(messages.id, ctx.params.messageId),
						});
						if (!message)
							return ctx.status("Not Found", {
								code: "messages.errors.messageNotFound",
							});
						if (message.authorId !== ctx.user?.userId)
				...
				.delete("/messages/:messageId", async (ctx) => {
					...
					if (ctx.user?.userId !== message.authorId)
						return ctx.status("Forbidden", {
							...
							JSON.stringify({
								...
								d: {
									...
									user: {
										...ctx.user?.user,
										member: {
											nickname: ctx.member?.nickname as string,
										},
									},
									timestamp: Date.now(),
						...
						return (
							await db
								...
								.values({
									...
									channelId: ctx.params.id,
									creatorId: ctx.user?.id as string,
								})
								.returning()
						)[0];
					} catch (e) {
