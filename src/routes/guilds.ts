...
const ServerCreateSchema = z.object({
	name: z
		.string("modals.serverCreate.nameRequired")
		.min(2, "modals.serverCreate.nameMinChars")
		.max(64, "modals.serverCreate.nameMaxChars"),
	brief: z
		.string("modals.serverCreate.briefRequired")
		.min(2, "modals.serverCreate.briefMinChars")
		.max(36, "modals.serverCreate.briefMinChars"),
});

export default new Elysia({ prefix: "/guilds" }).guard(
	{
		beforeHandle(ctx) {
			if (
				!isAuthenticated(
	...
	(app) =>
		app
			...
			.post("/", async (ctx) => {
				log(
					"Guilds",
					`Creating guild ${(ctx.body as any).name} for user ${ctx.user?.id}`,
				);
				const validatedBody = ServerCreateSchema.safeParse(ctx.body);
				if (!validatedBody.success) {
					log(
						"Guilds",
						`Creating guild ${(ctx.body as any).name} failed for user ${ctx.user?.id}: Validation failed. Stack trace below.`,
					);
				...
				const [result] = await db
					.select({ count: count() })
					.from(guildMembers)
					.where(eq(guildMembers.userId, ctx.user?.id as string));
				if (result.count >= Bun.env.USER_GUILD_LIMIT) {
					log(
						"Guilds",
						`Creating guild ${validatedBody.data.name} failed for user ${ctx.user?.id}: The user has too many guilds.`,
					);
				...
				try {
					...
					log(
						"Guilds",
						`Creating guild ${validatedBody.data.name} for user ${ctx.user?.id} succeeded. Assigned ID: ${id}`,
					);
