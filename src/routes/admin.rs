...
async fn gateway_metrics(State(state): State<SharedState>) -> Result<impl IntoResponse, AppError> {
    Ok(Json(
        state.gateway_metrics.snapshot(state.connected_users.len()),
    ))
}
...
async fn delete_invite_code(
    ...
    if invite.used {
        return Err(AppError::Forbidden(
            "Cannot delete used invite code".to_string(),
        ));
    }
