...
async fn register(
    ...
    if state.config.registration_closed {
        println!(
            "Registration closed. Value: {}",
            state.config.registration_closed
        );
    ...
    let existing_user = state
        ...
        .users
        .find_first(|q| {
            q.where_username(body.username.clone())
                .where_tag(tag.clone())
        })
        .await?;
    ...
    state
        .db
        .account_settings
        .create(|c| c.set_account_id(user_id.clone()))
        .await?;
...
async fn clear_db(State(state): State<SharedState>) -> Result<impl IntoResponse, AppError> {
    let is_production = std::env::var("NODE_ENV").unwrap_or_default() == "production";
