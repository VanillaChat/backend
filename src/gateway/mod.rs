...
async fn handle_socket(socket: WebSocket, state: SharedState, cookie_header: Option<String>) {
    ...
    if sender.send(Message::Text(serde_json::to_string(&hello).unwrap().into())).await.is_err() {
        return;
    }
                    ...
                    Ok(payload) => {
                        match payload.op {
                            ...
                            2 => {
                                ...
                                match events::identify::handle(&state_clone, &tx, token, &state_clone.config.token_secret).await {
                                    Ok(result) => {
                                        ...
                                        let mut existing = state_clone.connected_users
                                            .entry(result.user_id.clone())
                                            .or_insert_with(Vec::new);
                                        existing.push(tx.clone());
                                    }
                                    Err(e) => {
                                        let invalid = Payload::with_data(9, serde_json::json!(false));
    ...
    if let Some(uid) = user_id {
        ...
        if let Ok(members) = members {
            for member in members {
                let presence = Payload::dispatch("PRESENCE_UPDATE", serde_json::json!({
                    "userId": uid,
                    "status": "UNAVAILABLE"
                }));
