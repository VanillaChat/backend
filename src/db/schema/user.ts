...
export const accounts = pgTable("accounts", {
	...
	settingsId: text("settings_id")
		.unique()
		.notNull()
		.references(() => accountSettings.accountId, { onDelete: "cascade" }),
	inviteCode: text("invite_code")
		.unique()
		.references(() => inviteCodes.id, { onDelete: "no action" }),
});

export const accountsRelations = relations(accounts, ({ one }) => ({
	user: one(users, { fields: [accounts.userId], references: [users.id] }),
	settings: one(accountSettings, {
		fields: [accounts.settingsId],
		references: [accountSettings.accountId],
	}),
	inviteCode: one(inviteCodes, {
		fields: [accounts.inviteCode],
...
export const accountSettingsRelations = relations(
	accountSettings,
	({ one }) => ({
...
export const accountDeleteSchedule = pgTable("account_delete_schedule", {
	...
	deleteMessages: boolean("delete_messages").notNull().default(false),
	deleteAt: text("delete_at").notNull().default(""),
});
